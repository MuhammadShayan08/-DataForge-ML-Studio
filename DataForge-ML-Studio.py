...
def log_training(email: str, dataset: str, problem_type: str, best_model: str, score: float, rows: int, cols: int):
    ...
    history[email]["training_log"] = history[email]["training_log"][-200:]
    save_json(HISTORY_FILE, history)
...
[data-testid="stFileUploader"] > div:hover {{border-color:{"rgba(74,222,128,0.70)" if T=="dark" else "rgba(124,58,237,0.70)"} !important;background:{"#161616" if T=="dark" else "#e8e0ff"} !important;}}
[data-testid="stFileUploader"] label,[data-testid="stFileUploader"] span,[data-testid="stFileUploader"] p {{color:{"#6b7280" if T=="dark" else "#7c3aed"} !important;background:transparent !important;}}
[data-testid="stFileUploaderDropzone"] {{background:{"#0f0f0f" if T=="dark" else "#ede9fe"} !important;border:2px dashed {"rgba(74,222,128,0.35)" if T=="dark" else "rgba(124,58,237,0.40)"} !important;border-radius:12px !important;}}
[data-testid="stFileUploaderDropzone"]:hover {{background:{"#141414" if T=="dark" else "#e0d9ff"} !important;border-color:{"rgba(74,222,128,0.65)" if T=="dark" else "rgba(124,58,237,0.65)"} !important;}}
[data-testid="stFileUploaderDropzone"] * {{color:{"#6b7280" if T=="dark" else "#7c3aed"} !important;background:transparent !important;}}
[data-testid="stFileUploaderDropzone"] button {{background:{"#1a1a1a" if T=="dark" else "rgba(124,58,237,0.10)"} !important;border:1px solid {"rgba(74,222,128,0.35)" if T=="dark" else "rgba(124,58,237,0.45)"} !important;color:{"#4ade80" if T=="dark" else "#5b21b6"} !important;border-radius:8px !important;font-weight:600 !important;}}
...
div[data-testid="column"]:nth-child(2) .stButton>button {{background:{BG2 if T=="dark" else "#ffffff"} !important;color:{TEXT1} !important;border:1px solid {BORDER} !important;box-shadow:{"0 2px 8px rgba(0,0,0,0.4)" if T=="dark" else "0 2px 8px rgba(0,0,0,0.12)"} !important;font-weight:600 !important;min-width:90px !important;}}
div[data-testid="column"]:nth-child(2) .stButton>button:hover {{border-color:{ACCENT1} !important;color:{ACCENT1} !important;background:{"rgba(74,222,128,0.06)" if T=="dark" else "rgba(124,58,237,0.06)"} !important;transform:translateY(-2px) !important;}}
div[data-testid="column"]:nth-child(3) .stButton>button {{background:{BG2 if T=="dark" else "#ffffff"} !important;color:{TEXT1} !important;border:1px solid {BORDER} !important;box-shadow:{"0 2px 8px rgba(0,0,0,0.4)" if T=="dark" else "0 2px 8px rgba(0,0,0,0.12)"} !important;font-weight:600 !important;min-width:90px !important;}}
div[data-testid="column"]:nth-child(3) .stButton>button:hover {{border-color:{ACCENTR} !important;color:{ACCENTR} !important;background:rgba(248,113,113,0.06) !important;transform:translateY(-2px) !important;}}
...
[data-testid="collapsedControl"],button[data-testid="collapsedControl"] {{display:flex !important;visibility:visible !important;opacity:1 !important;pointer-events:all !important;background:{"#16a34a" if T=="dark" else "#5b21b6"} !important;border-radius:0 10px 10px 0 !important;min-width:24px !important;width:24px !important;}}
[data-testid="stSidebarCollapseButton"] button,button[kind="header"] {{visibility:visible !important;opacity:1 !important;background:{"rgba(74,222,128,0.12)" if T=="dark" else "rgba(124,58,237,0.12)"} !important;border:1px solid {"rgba(74,222,128,0.3)" if T=="dark" else "rgba(124,58,237,0.3)"} !important;border-radius:8px !important;color:{"#4ade80" if T=="dark" else "#7c3aed"} !important;}}
</style>
    ...
    <div class="sidebar-section" style="margin-bottom:.75rem">
      <div style="display:flex;align-items:center;gap:.6rem">
        ...
        <div style="min-width:0">
          ...
          <div style="font-size:.7rem;color:{"#6b7280" if T=="dark" else "#888"}">{uemail_global}</div>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.4rem;margin-top:.75rem">
        <div style="background:{"rgba(255,255,255,0.04)" if T=="dark" else "#f3eeff"};border-radius:8px;padding:.5rem .6rem;text-align:center">
          <div style="font-size:1.1rem;font-weight:900;color:{ACCENT2}">{uhist_sb.get('login_count',0)}</div>
          <div style="font-size:.6rem;color:{"#6b7280" if T=="dark" else "#888"};text-transform:uppercase;letter-spacing:.05em">Logins</div>
        </div>
        <div style="background:{"rgba(255,255,255,0.04)" if T=="dark" else "#f3eeff"};border-radius:8px;padding:.5rem .6rem;text-align:center">
          <div style="font-size:1.1rem;font-weight:900;color:{ACCENT1}">{uhist_sb.get('datasets_trained',0)}</div>
    ...
    with tab5:
        ...
        <div class="stat-grid slide-up" style="grid-template-columns:repeat(4,1fr)">
          ...
          <div class="stat-card">
            ...
            <div class="value" style="font-size:.9rem;padding-top:.3rem">{uhist_h.get('last_login','—')[:10] if uhist_h.get('last_login') else '—'}</div>
