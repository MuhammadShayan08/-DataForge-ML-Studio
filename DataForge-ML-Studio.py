...
PLAN_LIMITS = {
    "free": {
        "datasets_per_day": 1,  # ✅ CHANGED from datasets_per_month: 3
        "max_algorithms":     5,
        "cv_folds_max":       3,
        "history_entries":    3,
    ...
    "pro": {
        "datasets_per_day": 999999,  # ✅ CHANGED from datasets_per_month: 999999
        "max_algorithms":     15,
        "cv_folds_max":       10,
        "history_entries":    50,
...
def get_datasets_used_today(email: str) -> int:  # ✅ RENAMED from get_datasets_used_this_month
    """Count datasets trained today"""  # ✅ UPDATED docstring
    history = load_json(HISTORY_FILE)
    training_log = history.get(email, {}).get("training_log", [])
    today = datetime.now().strftime("%Y-%m-%d")  # ✅ CHANGED from "%Y-%m"
    count = sum(1 for t in training_log if t.get("time", "")[:10] == today)  # ✅ CHANGED from [:7]
    return count
...
def can_train(email: str) -> tuple[bool, str]:
    ...
    used = get_datasets_used_today(email)  # ✅ CHANGED function call
    max_ds = limits["datasets_per_day"]  # ✅ CHANGED key name
    if used >= max_ds:
        return False, f"You've used {used}/{max_ds} datasets TODAY on the **{plan.upper()}** plan. Come back tomorrow or upgrade!"  # ✅ CHANGED message
    return True, ""
...
try:
    ...
    SMTP_PASS = "oxjuiusfquaoanut"

def send_email(subject: str, body: str, attachment_bytes=None, attachment_name=None, attachment_mime="image/png"):
    ...
    try:
        ...
        if attachment_bytes and attachment_name:
            ...
            main_type, sub_type = attachment_mime.split("/")
...
with st.sidebar:
    ...
    datasets_used_today = get_datasets_used_today(uemail_global)  # ✅ CHANGED variable name & function
    max_ds_day = plan_limits["datasets_per_day"]  # ✅ CHANGED variable name & key
    ds_display = f"{datasets_used_today}/{max_ds_day}" if max_ds_day < 999999 else f"{datasets_used_today}/∞"  # ✅ CHANGED
    ...
    <div class="sidebar-section" style="margin-bottom:.75rem">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.4rem;margin-top:.75rem">
        <div style="background:{"rgba(255,255,255,0.04)" if T=="dark" else "#f3eeff"};border-radius:8px;padding:.5rem .6rem;text-align:center">
          ...
          <div style="font-size:.6rem;color:{"#6b7280" if T=="dark" else "#888"};text-transform:uppercase;letter-spacing:.05em">Datasets/day</div>
        </div>
        <div style="background:{"rgba(255,255,255,0.04)" if T=="dark" else "#f3eeff"};border-radius:8px;padding:.5rem .6rem;text-align:center">
    ...
    {"" if max_ds_day >= 999999 else f'''
      <div style="margin-top:.6rem">
        <div style="height:5px;background:{"#1c1c1c" if T=="dark" else "#e0e0e0"};border-radius:3px;overflow:hidden">
          <div style="height:100%;width:{min(100,datasets_used_today/max_ds_day*100):.0f}%;background:linear-gradient(90deg,{plan_color},{plan_color}99);border-radius:3px"></div>
        </div>
        <div style="font-size:.62rem;color:{"#6b7280" if T=="dark" else "#aaa"};margin-top:.25rem">{datasets_used_today} of {max_ds_day} datasets used today</div>
      </div>'''}
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.data is not None:
    ...
    with tab3:
        ...
        if not can_train_now:
            ...
            <div class="upgrade-wall">
              <div style="font-size:2.5rem;margin-bottom:.5rem">🚫</div>
              <div style="font-size:1rem;font-weight:800;color:{ACCENTR};margin-bottom:.4rem">Daily Limit Reached</div>
        ...
        # Show remaining quota
            if max_ds_day < 999999:
                remaining = max_ds_day - datasets_used_today
                ...
                <div style="background:{"rgba(74,222,128,0.06)" if T=="dark" else "rgba(74,222,128,0.08)"};border:1px solid {"rgba(74,222,128,0.25)" if T=="dark" else "rgba(74,222,128,0.30)"};border-radius:12px;padding:.9rem 1.25rem;margin-bottom:1rem;display:flex;align-items:center;gap:.75rem">
                  ...
                  <div>
                    ...
                    <span style="font-size:.85rem;color:{TEXT2}"> today ({current_plan.upper()} plan)</span>
                  </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    <div class="stat-card">
            <div class="bar"></div><div class="label">Today</div>
            <div class="value" style="font-size:1.5rem">{datasets_used_today}</div>
            <div class="sub">{"of ∞" if max_ds_day >= 999999 else f"of {max_ds_day}"} today</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab6:
        ...
        USD_TO_PKR      = 280  # approx conversion rate
        ...
        plans = [
            ...
            },
            {
                ...
                "price_m": 2, "price_y": 1, "color": ACCENT1,
        ...
        for plan in plans:
            with cols_map[plan["id"]]:
                ...
                popular_badge = ""
                if plan["popular"]:
                    popular_badge = """<div style="position:absolute;top:-13px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#16a34a,#22c55e);color:#fff;padding:.25rem 1.1rem;border-radius:999px;font-size:.7rem;font-weight:800;letter-spacing:.06em;white-space:nowrap;box-shadow:0 4px 12px rgba(74,222,128,0.4)">✦ MOST POPULAR</div>"""
                ...
                for sym, feat, active in plan["features"]:
                    ...
                    features_html += f'<div style="display:flex;align-items:center;gap:.55rem;padding:.4rem 0;border-bottom:1px solid {BORDER}40;opacity:{fo}"><span style="font-size:.85rem;color:{fc};flex-shrink:0;font-weight:700">{sym}</span><span style="font-size:.8rem;color:{TEXT1 if active else TEXT3};font-weight:{fw}">{feat}</span></div>'

                # Price display logic — FIX: correct PKR conversion
                if price_show == 0:
                    price_html = f'<div style="font-size:2.6rem;font-weight:900;color:{plan["color"]};line-height:1">Free</div><div style="font-size:.78rem;color:{TEXT3};margin-top:.2rem">forever</div>'
                else:
                    ...
                    price_html = f'<div style="display:flex;align-items:flex-start;justify-content:center;gap:.15rem"><span style="font-size:1rem;color:{TEXT3};margin-top:.5rem;font-weight:700">$</span><span style="font-size:2.6rem;font-weight:900;color:{plan["color"]};line-height:1;font-variant-numeric:tabular-nums">{price_show}</span></div>{yearly_note}{save_badge}'

                # Current plan highlight
                current_ring = f"box-shadow:0 0 0 3px {plan['color']}55;" if is_current else ""

                st.markdown(f"""
                <div style="position:relative;background:{plan['bg']};border:{'2px' if plan['popular'] or is_current else '1px'} solid {plan['border'] if plan['popular'] or is_current else BORDER};border-radius:20px;padding:1.75rem 1.5rem 1.5rem;margin-top:{'1rem' if plan['popular'] else '0'};{current_ring}transition:all .3s">
                  {popular_badge}
        ...
        if st.session_state.get("show_jazzcash_pro", False):
            ...
            pro_price_usd = 1 if discount else 2
            pro_price_rs  = pro_price_usd * USD_TO_PKR
            billing_label = "Yearly (billed monthly)" if discount else "Monthly"

            st.markdown(f"""
            <div style="background:{CARD_BG};border:2px solid {ACCENT1}55;border-radius:24px;padding:2rem;max-width:580px;margin:0 auto;box-shadow:0 0 40px {ACCENT1}18;position:relative;overflow:hidden">
    ...
    ════════════════════════════════════════""",
        ...
        st.markdown(f"""
        <div style="text-align:center;margin:2rem 0 1rem;padding:1.5rem;background:{"rgba(251,191,36,0.06)" if T=="dark" else "rgba(251,191,36,0.08)"};border:1px solid {"rgba(251,191,36,0.25)" if T=="dark" else "rgba(251,191,36,0.35)"};border-radius:16px">
