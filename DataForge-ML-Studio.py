...
TOKENS_FILE  = "dataforge_tokens.json"
PAYMENTS_FILE  = "dataforge_payments.json"   # NEW: Payment records
ADMIN_EMAILS   = {"shayan.corner@gmail.com"}  # Add more admin emails here
            ...
            with adm4:
                ...
                if not email_log:
                    ...
                    for ts, entry in log_entries:
                        ...
                        st.markdown(f"""
                        <div style="background:{CARD_BG};border:1px solid {"rgba(248,113,113,0.30)" if has_error else BORDER};
                          ...
                          <div style="display:flex;align-items:flex-start;gap:.75rem">
                            ...
                            <div style="flex:1;min-width:0">
                              ...
                              {f'<div style="font-size:.75rem;color:{ACCENTR};margin-top:.35rem;background:rgba(248,113,113,0.07);border-radius:6px;padding:.3rem .6rem"><b>Error:</b> {error_txt}</div>' if error_txt else ""}
                              {f'<div style="font-size:.72rem;color:{ACCENTY};margin-top:.3rem;background:rgba(251,191,36,0.07);border-radius:6px;padding:.3rem .6rem">{note_txt}</div>' if note_txt else ""}
                            </div>
                          </div>
                        </div>""", unsafe_allow_html=True)
